...
class Navigator(object, metaclass=Singleton):
    ...
    def _get_soup(self, url: str, manual: bool = False) -> BeautifulSoup:
        """Return the BeautifulSoup for a page on scholar.google.com"""
        # # # # # # # # # WARNING # # # # # # # # # # #
        # # # # # # CUSTOM MANUAL INPUT MODE  # # # # #
        final_url = _HOST.format(url)
        if not manual:
            html = self._get_page(final_url)
        else:
            pyperclip.copy(final_url) # copy URL to user's clipboard
            print('URL copied to clipboard -- please enter HTML')
            html = ""
            while True:
                piece = input('>')
                html += piece
    ...
    def search_authors(self, url: str, manual: bool = False):
        """Generator that returns Author objects from the author search page"""
        soup = self._get_soup(url, manual)
    ...
    def search_publications(self, url: str, manual: bool = False):
        """Generator that returns Publication objects from the publication search page"""
        soup = self._get_soup(url, manual)

        while True:
            rows = soup.find_all('div', class_='gs_r gs_or gs_scl')
            self.logger.info("Found %d publications", len(rows))
            for row in rows:
                yield Publication(self, row, 'scholar')
            ...
            if soup.find(class_='gs_ico gs_ico_nav_next'):
                ...
                self.logger.info("No more publication pages")
                break
            ...
            next_button = soup.find(class_=cls1+cls2)  # Can be improved
            if next_button and 'disabled' not in next_button.attrs:
                self.logger.info("Loading next page of publications")
                url = next_button['onclick'][17:-1]
                url = codecs.getdecoder("unicode_escape")(url)[0]
                soup = self._get_soup(url)
            else:
